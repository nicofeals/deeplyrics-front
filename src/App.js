  ...
  getLyrics(e) {
    ...
    fetch('http://127.0.0.1:5000/lyrics?' + queryString, requestOptions)
      ...
      .then((data) => {
        ...
        res = res.toLowerCase().split('.').map(word => word.charAt(0).toUpperCase() + word.substring(1)).join('.');
            ...
            <Space
              ...
              <div className="band-logos">
                <Space direction='horizontal' size='large'>
                  {bands.map((image, index) => <Card key={index} hoverable={true} bordered={false}><Image src={image} width={150} preview={false} /></Card>)}
                </Space>
              </div>
              <div className="radio">
              ...
              {(lyrics !== '' || loading === true) &&
                <div className="lyrics">
                  <Card bordered={false} loading={loading}>
                    <Title level={5} style={{ whiteSpace:"pre-wrap", display: 'flex', alignSelf: 'center', justifyContent: 'center' }}>{lyrics}</Title>
                  </Card>
                </div>
              }
            </Space>
